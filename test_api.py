...
st, d = call("POST", "/api/payments", {"client_id": CLIENT, "amount": 100,
                                       "kind": "Police clearance fee"}, token=T)
ok("a payment type typed under Other is accepted", st == 200, (st, d))
st, d = call("GET", "/api/payments?from=2000-01-01&to=2030-01-01", token=T)
ok("  and stored exactly as written",
   any(p["kind"] == "Police clearance fee" for p in d["payments"]),
   [p["kind"] for p in d["payments"]][:4])
...
ok("an administrator sees the client list", st == 200, st)
for path in ["/api/reports", "/api/system", "/api/audit", "/api/users"]:
    st, d = call("GET", path, token=T2)
    if st != 403:
        ok("the office section is closed to them (%s)" % path, False, st)
        break
else:
    ok("the whole office section is closed to an administrator", True)
st, d = call("GET", "/export/clients?from=2000-01-01&to=2030-01-01", token=T2)
ok("  including the exports", st == 403, st)
...
ok("it lists every table's row count", len(d["primary"]["counts"]) == 23,
