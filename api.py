...
def list_invoices(ctx):
    ...
    return {"invoices": rows, "base": core.BASE_CURRENCY,
...
def create_invoice(ctx):
    ...
    currency = (ctx.s("currency") or core.BASE_CURRENCY).upper()
    ...
    if inv["client_id"] and ctx.b("record_payment", True):
        pay_id = ctx.conn.insert(
            ...
            (inv["client_id"], inv["case_id"], inv["total"], inv["currency"],
             ...
             ctx.s("paid_on") or today(), "", ctx.user["id"], now()))
...
def accounts_summary(ctx):
    ...
    missing = sorted({r["currency"] for r in rows
                      if r["currency"] != core.BASE_CURRENCY
                      and not r["fx_rate"]}
                     | {p["currency"] for p in payments
                        if core.to_base(ctx.conn, 1, p["currency"]) is None})
    return {"from": frm, "to": to, "base": core.BASE_CURRENCY,
...
def fx_list(ctx):
    need_admin(ctx)
    s = core.get_settings(ctx.conn)
    return {"base": core.BASE_CURRENCY, "rates": core.rates_table(ctx.conn),
...
def get_settings_api(ctx):
    ...
    out["base_currency"] = core.BASE_CURRENCY
    if ctx.user["role"] == "owner":
