...
FIELD_MAP = {
    ...
    "permit_status": ["permit_status", "current_status", "visa_status",
                      "currentPermitStatus"],
    "years_in_lesotho": ["years_in_lesotho", "years", "yearsInLesotho"],
    "criminal_record": ["criminal_record", "criminal", "convictions"],
    "prior_rejection": ["prior_rejection", "previous_rejection", "rejected"],
...
def public_intake(ctx):
    ...
    ref = core.next_ref(ctx.conn, "enquiries", "ENQ")
    ctx.conn.execute(
        ...
        (ref, vals["first_name"][:120], vals["last_name"][:120],
         ...
         1 if ctx.body.get("consent") else 0,
...
def convert_enquiry(ctx, eid):
    ...
    ref = core.next_ref(ctx.conn, "clients", "CL")
    client_id = ctx.conn.insert(
...
def create_client(ctx):
    ...
    ref = core.next_ref(ctx.conn, "clients", "CL")
    vals = [ref, first, last] + [ctx.s(f) for f in CLIENT_FIELDS[2:-1]]
    cid = ctx.conn.insert(
        "INSERT INTO clients (ref, first_name, last_name, email, phone, "
...
def _create_case(ctx, client_id, service, destination="", advisor_id=None,
    ...
    ref = core.next_ref(ctx.conn, "cases", "CS")
    s = core.get_settings(ctx.conn)
    case_id = ctx.conn.insert(
