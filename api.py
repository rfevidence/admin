...
@route("POST", r"/api/payments/(\d+)/void")
def void_payment(ctx, pid):
    """Mark a payment as reversed, keeping it on the record."""
    need_admin(ctx)
    pid = int(pid)
    p = ctx.conn.one("SELECT * FROM payments WHERE id = ?", (pid,))
    if not p:
        raise ApiError(404, "That payment no longer exists.")
    ctx.conn.execute("UPDATE payments SET voided = ? WHERE id = ?",
                     (0 if p["voided"] else 1, pid))
    core.audit(ctx.conn, ctx.user,
               "payment.unvoided" if p["voided"] else "payment.voided",
               f"{p['amount']} {p['currency']}", ctx.ip)
    ctx.conn.commit()
    return {"ok": True, "voided": not p["voided"]}
...
def analytics(ctx):
    ...
    by_stage = [{"name": s, "n": ctx.conn.scalar(
        "SELECT COUNT(*) AS n FROM cases WHERE closed = 0 AND stage = ?", (s,))}
    ...
    trend = [{
        "month": m,
        "enquiries": ctx.conn.scalar(
            "SELECT COUNT(*) AS n FROM enquiries WHERE created_at LIKE ?", (m + "%",)),
        "cases": ctx.conn.scalar(
            "SELECT COUNT(*) AS n FROM cases WHERE opened_at LIKE ?", (m + "%",)),
        "clients": ctx.conn.scalar(
            "SELECT COUNT(*) AS n FROM clients WHERE created_at LIKE ?", (m + "%",)),
    } for m in reversed(months)]
    ...
    return {
        ...
        "totals": {
            ...
            "cases": ctx.conn.scalar("SELECT COUNT(*) AS n FROM cases"),
            "open_cases": ctx.conn.scalar(
                "SELECT COUNT(*) AS n FROM cases WHERE closed = 0"),
...
def delete_invoice(ctx, iid):
    ...
    core.audit(ctx.conn, ctx.user, inv["kind"] + ".deleted", inv["number"], ctx.ip)
    ctx.conn.commit()
    return {"ok": True}
...
def accounts_summary(ctx):
    ...
    ledger = round(sum((core.to_base(ctx.conn, p["total"], p["currency"]) or 0)
                       for p in payments), 2)

    # Anything the books could not convert is named, so a total is never
    # quietly short.
    missing = sorted({r["currency"] for r in rows
                      if r["currency"] != core.base_currency(ctx.conn)
                      and not r["fx_rate"]}
                     | {p["currency"] for p in payments
                        if core.to_base(ctx.conn, 1, p["currency"]) is None})
    return {"from": frm, "to": to, "base": core.base_currency(ctx.conn),
