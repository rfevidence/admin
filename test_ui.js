...
(async function () {
  ...
  const withCss = html.replace('<link rel="stylesheet" href="/app.css" />',
    '<style>' + css + '</style>');
  const dom = new JSDOM(withCss.replace('<script src="/app.js"></script>', ''), {
  ...
  // The hidden attribute works only because the browser's own stylesheet says
  // [hidden]{display:none}. Any author rule setting display on the same
  // element outranks that and leaves it on screen — which is how an empty
  // dialog once sat over the sign-in page. jsdom does not model this cascade,
  // so check the stylesheet itself rather than a computed value.
  const hiddenEls = [...html.matchAll(/<[^>]*id="([A-Za-z0-9_-]+)"[^>]*\bhidden\b[^>]*>/g)]
    .map(m => m[1]);
  ok('the shell still marks elements hidden', hiddenEls.length >= 3, hiddenEls);

  const classOf = {};
  for (const id of hiddenEls) {
    const tag = html.match(new RegExp('<[^>]*id="' + id + '"[^>]*>'));
    const cls = tag && tag[0].match(/class="([^"]*)"/);
    classOf[id] = cls ? cls[1].split(/\s+/).filter(Boolean) : [];
  }
  const setsDisplay = [];
  for (const [id, classes] of Object.entries(classOf)) {
    for (const c of classes.concat([])) {
      const rule = new RegExp('\\.' + c + '\\s*\\{[^}]*display\\s*:', 'm');
      if (rule.test(css)) setsDisplay.push('#' + id + ' (.' + c + ')');
    }
  }
  const override = /\[hidden\]\s*\{[^}]*display\s*:\s*none\s*!important/.test(css);
  ok('any element whose class sets display is protected by a [hidden] override',
    setsDisplay.length === 0 || override,
    'these rely on it: ' + setsDisplay.join(', '));
  ok('the [hidden] override is present in the stylesheet', override);
