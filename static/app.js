...
function apptForm(link, after) {
  ...
  modal('Book a consultation',
    (needsClient
      ? '<div class="field"><label class="lbl" for="aClientQ">Who is it for</label>' +
        '<input class="inp" id="aClientQ" placeholder="Start typing their name" ' +
        'autocomplete="off">' +
        '<select class="inp" id="aClient" size="4" hidden></select>' +
        '<div class="hint" id="aClientHint">Search by name, reference or ' +
        'passport number.</div></div>'
  ...
  var chosenClient = link.client_id || null;
  ...
  if (needsClient) {
    var box = $('aClientQ'), list = $('aClient'), hint = $('aClientHint'), timer;
        ...
        api('/api/clients?q=' + encodeURIComponent(q)).then(function (r) {
          ...
          list.innerHTML = found.map(function (c) {
            return '<option value="' + c.id + '">' +
              esc(c.first_name + ' ' + c.last_name) + ' — ' + esc(c.ref) +
              '</option>';
          }).join('');
          list.hidden = false;
    ...
    list.onchange = function () {
      chosenClient = list.value;
      box.value = list.options[list.selectedIndex].textContent.split(' — ')[0];
      list.hidden = true;
      hint.textContent = 'Booking for ' + box.value + '.';
    };
  }

  $('aGo').onclick = function () {
    if (needsClient && !chosenClient) {
