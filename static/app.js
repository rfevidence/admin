...
function viewDashboard() {
  ...
  api('/api/dashboard').then(function (d) {
    ...
    html += '<div class="card"><h3>Enquiries and cases, last 12 months</h3>' +
      trendSvg(d.trend) +
      '<div class="legend"><span><i style="background:var(--brand-deep)"></i>Enquiries</span>' +
      '<span><i style="background:var(--brand-gold)"></i>Cases opened</span></div></div>';
...
function trendSvg(trend) {
  ...
  return '<svg class="trend" viewBox="0 0 ' + W + ' ' + H + '" preserveAspectRatio="none" ' +
    ...
    '" y2="' + (H - pad) + '"/>' +
    '<text x="' + pad + '" y="' + (H - 5) + '" font-size="10" fill="#64738A">' +
    esc(trend[0] ? trend[0].month : '') + '</text>' +
    '<text x="' + (W - pad) + '" y="' + (H - 5) + '" font-size="10" fill="#64738A" ' +
    'text-anchor="end">' + esc(trend.length ? trend[trend.length - 1].month : '') + '</text>' +
    '<text x="' + pad + '" y="12" font-size="10" fill="#64738A">peak ' + maxV + '</text></svg>';
}
