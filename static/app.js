...
function money(n, cur) {
  var v = Number(n || 0);
  // The system currency comes from Settings; org.currency is the older name
  // for the same thing and the server keeps them in step.
  var code = cur || (BOOT && BOOT.base_currency) ||
    (BOOT && BOOT.org && BOOT.org.currency) || 'USD';
...
function selectOther(id, list, value, blank) {
  ...
  // The "other" option may be spelled Other or other depending on the list,
  // so find whichever this one uses rather than assuming.
  var otherValue = 'Other';
  (list || []).forEach(function (o) {
    var v = String(typeof o === 'object' ? o.value : o);
    if (/^other$/i.test(v)) otherValue = v;
  });
  return '<select class="inp" id="' + id + '" data-other="' + id + '_o">' +
    opts(list, isOther ? otherValue : value, blank) + '</select>' +
  ...
  function attempt(n) {
    return fetch(path, opt).then(function (r) {
      ...
      return r.json().catch(function () { return {}; }).then(function (d) {
        if (r.status === 202 && d && d.pending_approval) {
          // Not an error: the change is real, it is simply waiting for the
          // owner. Saying so once here means every screen says it.
          toast(d.message || 'Sent to the owner to approve.', 'ok');
          PENDING_MINE = (PENDING_MINE || 0) + 1;
          paintChrome();
          return d;
        }
...
/* login.png is optional. The server marks the page when it is actually
   deployed, so there is nothing to probe and nothing a cached reply can get
   wrong. Without it the brand gradient stands alone and the text inverts. */
(function () {
  var panel = $('signin');
  if (!panel) return;
  if (document.documentElement.getAttribute('data-photo') === '1') {
...
function paintChrome() {
  ...
  NAV.forEach(function (group) {
    ...
    group[1].forEach(function (it) {
      ...
      if (it[0] === 'approvals' && PENDING_MINE) {
        pill = '<span class="pill" id="navPending">' + PENDING_MINE + '</span>';
      }
...
function refreshUnread() {
  api('/api/bootstrap').then(function (d) {
    ...
    PENDING_MINE = d.pending_approvals || 0;
    APPROVALS_ON = d.approvals_on !== false;
    paintChrome();
...
function paymentForm(link, after) {
  ...
  modal('Record a payment',
    ...
    selectOther('pKind', BOOT.payment_kinds.map(function (k) {
  ...
  $('pGo').onclick = function () {
    ...
    savePayment({
      ...
      amount: num('pAmount'), kind: valOther('pKind'), method: valOther('pMethod'),
...
function viewSettings() {
  ...
  api('/api/settings').then(function (d) {
    ...
    var mailReady = !!(s.smtp_host && s.smtp_user && s.smtp_pass_set);
    html += '<div class="card"><h3>Consultation reminders' +
      (s.reminders_enabled === '1'
        ? ' <span class="badge ok">on</span>'
        : ' <span class="badge grey">off</span>') +
      (mailReady ? ' <span class="badge ok">email ready</span>'
                 : ' <span class="badge warn">email not set up</span>') + '</h3>' +
      (mailReady ? '' :
        '<div class="notice warn" style="margin:0 0 12px">Still needed: ' +
        [!s.smtp_host ? 'the mail server' : '',
         !s.smtp_user ? 'the username' : '',
         !s.smtp_pass_set ? 'the app password' : '']
          .filter(function (x) { return x; }).join(', ') +
        '. Fill them in below — <b>Send a test message</b> saves them for you ' +
        'and then tries to send.</div>') +
    ...
    html += '<div class="card"><h3>Owner approval' +
      (s.require_approval === '0'
        ? ' <span class="badge grey">off</span>'
        : ' <span class="badge ok">on</span>') + '</h3>' +
      '<p class="muted small">With this on, every change an administrator ' +
      'makes waits for you to agree to it, and appears under Approvals. ' +
      'Reading is never held. Signing in, changing their own password and ' +
      'setting up their own two-step security never wait either — otherwise ' +
      'they could not get into the portal to do the work.</p>' +
      '<div class="field" style="max-width:320px">' +
      '<label class="lbl" for="sApproval">Administrators</label>' +
      '<select class="inp" id="sApproval"' + (ro ? ' disabled' : '') + '>' +
      opts([{ value: '1', label: 'Need your approval for every change' },
            { value: '0', label: 'Work directly, no approval needed' }],
        s.require_approval || '1') + '</select>' +
      '<div class="hint">Worth knowing: with approval on, a client sitting in ' +
      'the office cannot be added until you agree, so somebody has to be ' +
      'available to press the button.</div></div></div>';
    ...
    if ($('sSave')) {
      $('sSave').onclick = function () {
        ...
        api('/api/settings', {
          ...
          require_approval: val('sApproval'),
    ...
    if ($('sTestMail')) {
      /* The test reads what is stored, not what is on screen. Saving first
         means a value you have just typed is actually the one tested — the
         old behaviour quietly tested the blanks. */
      $('sTestMail').onclick = function () {
        ...
        api('/api/settings', {
          ...
          var pw = $('sSmtpPass') ? $('sSmtpPass').value : '';
          if (!pw) return null;
          return api('/api/settings/smtp-password', { password: pw })
            .then(function () { $('sSmtpPass').value = ''; });
        }).then(function () {
          b.textContent = 'Sending…';
          return api('/api/settings/test-email', {});
        }).then(function (r) {
          toast('Test message sent to ' + r.sent_to + '. Check that inbox.', 'ok');
        }).catch(function (e) { toast(e.message, 'bad'); })
          .then(function () {
...
function serviceForm(s) {
  modal(s ? 'Edit service' : 'Add a service',
    '<div class="field"><label class="lbl" for="svName">What it is called</label>' +
    '<input class="inp" id="svName" placeholder="Police Clearance Assistance" ' +
    'value="' + esc(s ? s.service : '') + '"></div>' +
    (s ? '<label class="small"><input type="checkbox" id="svActive"' +
      (s.active === false ? '' : ' checked') + '> Still offered ' +
      '(uncheck to retire it)</label>' +
      '<p class="muted small" style="margin-top:10px">Renaming does not touch ' +
      'cases already open — their service is the wording agreed with that ' +
      'client at the time.</p>' : ''),
    '<button class="btn" onclick="closeModal()">Cancel</button>' +
    (s && ME.role === 'owner'
      ? '<button class="btn btn-danger left" id="svDel">Delete</button>' : '') +
    '<button class="btn btn-primary" id="svGo">Save</button>');

  $('svGo').onclick = function () {
    if (!val('svName')) { toast('Give the service a name.', 'bad'); return; }
    this.disabled = true;
    var body = { name: val('svName') };
    if (s) body.active = checked('svActive');
    var p = s ? api('/api/services/list/' + s.id, body)
              : api('/api/services/list', body);
    p.then(function () {
      closeModal(); toast('Saved.', 'ok');
      if (s && svcPick === s.service) svcPick = val('svName');
      refreshBoot(function () { viewServices(); });
    }).catch(function (e) { toast(e.message, 'bad'); $('svGo').disabled = false; });
  };

  if ($('svDel')) {
    $('svDel').onclick = function () {
      var go = function (force) {
        api('/api/services/list/' + s.id, force ? { force: true } : {}, 'DELETE')
          .then(function () {
            closeModal(); toast('Service removed.');
            if (svcPick === s.service) svcPick = null;
            refreshBoot(function () { viewServices(); });
          })
          .catch(function (e) {
            if (/still use/.test(e.message)) {
              confirmAction('Cases still use this service',
                e.message + ' Deleting anyway leaves those cases naming a ' +
                'service that is no longer on the list. Their requirements ' +
                'and fees go too.', 'Delete it anyway', function () { go(true); });
              return;
            }
            toast(e.message, 'bad');
          });
      };
      confirmAction('Remove ' + s.service + '?',
        'Retiring it instead keeps it for the cases that used it while hiding ' +
        'it from the lists.', 'Remove', function () { go(false); });
    };
  }
}

/* The service list feeds dropdowns all over the portal, so refresh it. */
    ...
    var html = '<p class="muted small" style="margin:0 0 13px">' +
      ...
      '<div class="svc-list">' + d.services.map(function (s) {
        return '<button data-svc="' + esc(s.service) + '" class="' +
          ...
          '</span>' + (s.id
            ? '<span class="svc-edit" data-svcedit="' + s.id + '">Edit</span>'
            : '') + '</button>';
      }).join('') + '</div>' +
      '<button class="btn btn-sm btn-gold" id="svcAdd" style="margin-top:10px">' +
      'Add a service</button></div><div id="svcDetail"><div class="loading">Loading…</div></div></div>';
    $('view').innerHTML = html;
    document.querySelectorAll('[data-svc]').forEach(function (b) {
      b.onclick = function () { svcPick = b.dataset.svc; viewServices(); };
    });
    if ($('svcAdd')) $('svcAdd').onclick = function () { serviceForm(null); };
    document.querySelectorAll('[data-svcedit]').forEach(function (b) {
      b.onclick = function (ev) {
        ev.stopPropagation();
        serviceForm(d.services.filter(function (s) {
          return String(s.id) === b.dataset.svcedit; })[0]);
      };
    });
  ...
  api('/api/invoices/' + id).then(function (d) {
    ...
    if (v.kind === 'invoice' && v.status !== 'paid' && v.status !== 'cancelled') {
      foot += '<button class="btn" id="invEdit">Edit</button>' +
        '<button class="btn" id="invCancel">Cancel it</button>' +
        '<button class="btn btn-gold" id="invPaid">Mark paid and raise a receipt</button>';
    }
    if (ME.role === 'owner') {
      foot += '<button class="btn btn-danger" id="invDel">Delete</button>';
    }
    ...
    if ($('invCancel')) {
      /* Cancelling is nearly always the right answer for a document that has
         already left the office: the number stays used, so the book has no
         hole in it, and the amount drops out of every total. */
      $('invCancel').onclick = function () {
        confirmAction('Cancel invoice ' + v.number + '?',
          'It stays in the book with its number, marked cancelled, and comes ' +
          'out of the totals. That is what an accountant expects to see — a ' +
          'missing number raises questions a cancelled one does not.',
          'Cancel the invoice', function () {
            api('/api/invoices/' + v.id, { status: 'cancelled' }).then(function () {
              closeModal(); toast('Invoice cancelled.'); viewAccounts();
            }).catch(function (e) { toast(e.message, 'bad'); });
          });
      };
    }
    if ($('invDel')) {
      $('invDel').onclick = function () {
        var isReceipt = v.kind === 'receipt';
        confirmAction('Delete ' + v.kind + ' ' + v.number + ' for good?',
          (isReceipt
            ? 'The payment it posted stays in the ledger — that is the record ' +
              'of money actually received, and this only removes the paperwork. ' +
              'Remove the payment separately on the Payments page if the money ' +
              'never arrived. '
            : 'Cancelling is usually better: it leaves the number in the book ' +
              'rather than a gap. ') +
          'The number ' + v.number + ' will not be used again either way, and ' +
          'the deletion is recorded in the activity log.',
          'Delete it', function () {
            api('/api/invoices/' + v.id, {}, 'DELETE').then(function () {
              closeModal();
              toast(v.kind.charAt(0).toUpperCase() + v.kind.slice(1) +
                    ' ' + v.number + ' deleted.');
              viewAccounts();
            }).catch(function (e) { toast(e.message, 'bad'); });
          });
...
/* ============================================================ APPROVALS */
var apprFilter = 'pending';

function viewApprovals() {
  setActive('approvals', ME.role === 'owner' ? 'Approvals' : 'My requests');
  loading('Looking for anything waiting…');
  api('/api/approvals?status=' + apprFilter).then(function (d) {
    var owner = ME.role === 'owner';
    var html = '<p class="muted small" style="margin:0 0 13px">' +
      (owner
        ? 'Changes made by administrators wait here until you agree to them. ' +
          'Reading is never held — only changes.'
        : 'Changes you have made that are waiting for the account owner. ' +
          'They take effect once approved.') +
      (d.enabled ? '' : ' <b>Approval is currently switched off, so changes ' +
        'take effect immediately.</b>') + '</p>';

    html += '<div class="tabs">' + [
      ['pending', 'Waiting'], ['approved', 'Approved'],
      ['rejected', 'Turned down'], ['all', 'Everything']
    ].map(function (t) {
      return '<button data-af="' + t[0] + '" class="' +
        (apprFilter === t[0] ? 'on' : '') + '">' + t[1] +
        (t[0] === 'pending' && d.pending ? ' (' + d.pending + ')' : '') +
        '</button>';
    }).join('') + '</div>';

    html += '<div class="card">' + (d.approvals.length
      ? '<ul class="docs">' + d.approvals.map(function (a) {
          return '<li><div class="doc-name"><b>' + esc(a.summary) + '</b>' +
            apprBadge(a.status) +
            '<span class="sub">asked by ' + esc(a.requested_by_name || 'someone') +
            ' · ' + ago(a.requested_at) +
            (a.decided_at ? ' · decided by ' + esc(a.decided_by_name || '—') +
              ' ' + ago(a.decided_at) : '') +
            (a.note ? '<br>' + esc(a.note) : '') +
            '</span></div>' +
            (a.status === 'pending' && owner
              ? '<button class="btn btn-sm btn-gold" data-ayes="' + a.id +
                '">Approve</button>' +
                '<button class="btn btn-sm btn-danger" data-ano="' + a.id +
                '">Turn down</button>'
              : '') +
            (a.status === 'pending' && !owner
              ? '<button class="btn btn-sm" data-awd="' + a.id +
                '">Withdraw</button>' : '') +
            '<button class="btn btn-sm" data-asee="' + a.id + '">Details</button>' +
            '</li>';
        }).join('') + '</ul>'
      : emptyBox(apprFilter === 'pending' ? 'Nothing waiting'
                                          : 'Nothing to show',
          owner ? 'Changes needing your agreement appear here.'
                : 'Anything you change that needs the owner will appear here.')) +
      '</div>';

    $('view').innerHTML = html;
    document.querySelectorAll('[data-af]').forEach(function (b) {
      b.onclick = function () { apprFilter = b.dataset.af; viewApprovals(); };
    });
    document.querySelectorAll('[data-ayes]').forEach(function (b) {
      b.onclick = function () { decideApproval(b.dataset.ayes, 'approve'); };
    });
    document.querySelectorAll('[data-ano]').forEach(function (b) {
      b.onclick = function () { decideApproval(b.dataset.ano, 'reject'); };
    });
    document.querySelectorAll('[data-awd]').forEach(function (b) {
      b.onclick = function () {
        api('/api/approvals/' + b.dataset.awd, {}, 'DELETE').then(function () {
          toast('Withdrawn.'); viewApprovals();
        }).catch(function (e) { toast(e.message, 'bad'); });
      };
    });
    document.querySelectorAll('[data-asee]').forEach(function (b) {
      b.onclick = function () {
        var a = d.approvals.filter(function (x) {
          return String(x.id) === b.dataset.asee; })[0];
        modal('What was asked for',
          '<dl class="kv">' + kvRow('Change', a.summary) +
          kvRow('Asked by', a.requested_by_name) +
          kvRow('When', dtm(a.requested_at)) +
          kvRow('Status', a.status) + '</dl>' +
          '<span class="lbl">The detail</span><div class="code">' +
          esc(JSON.stringify(a.body, null, 2)) + '</div>',
...
function apprBadge(s) {
  var m = { pending: ['warn', 'waiting'], approved: ['ok', 'approved'],
            rejected: ['bad', 'turned down'], withdrawn: ['grey', 'withdrawn'] };
  var x = m[s] || ['grey', s];
  return ' <span class="badge ' + x[0] + '">' + esc(x[1]) + '</span>';
}
...
function decideApproval(id, decision) {
  var doIt = function (note) {
    api('/api/approvals/' + id + '/decide', { decision: decision, note: note || '' })
      .then(function () {
        toast(decision === 'approve' ? 'Approved and applied.' : 'Turned down.',
              decision === 'approve' ? 'ok' : '');
        viewApprovals();
      }).catch(function (e) { toast(e.message, 'bad'); });
  };
  if (decision === 'approve') { doIt(''); return; }
  modal('Turn this down',
    '<div class="field"><label class="lbl" for="apNote">Why (optional)</label>' +
    '<input class="inp" id="apNote" placeholder="So they know what to change"></div>',
    '<button class="btn" onclick="closeModal()">Cancel</button>' +
    '<button class="btn btn-danger" id="apGo">Turn it down</button>');
  $('apGo').onclick = function () { var n = val('apNote'); closeModal(); doIt(n); };
}
