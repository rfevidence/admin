...
function clientFields(c) {
  ...
  return '<div class="grid-2">' +
    ...
    '<select class="inp" id="cfPermit">' + optsKeeping(BOOT.permit_statuses,
      c.permit_status, '—') +
    '</select></div>' +
    '<div class="field"><label class="lbl" for="cfPermitExp">' +
    'Permit or visa expires</label>' +
    '<input class="inp" id="cfPermitExp" type="date" value="' +
    esc(c.permit_expiry || '') + '">' +
...
function caseEditForm(k) {
  modal('Edit ' + k.ref,
    ...
    '<select class="inp" id="keService">' + optsKeeping(BOOT.services, k.service) +
    '</select></div>' +
    '<div class="grid-2">' +
...
function viewSettings() {
  ...
  api('/api/settings').then(function (d) {
    ...
    html += '<div class="card"><h3>Client case tracking' +
      ...
      '<label class="lbl" for="sTrackOn">Tracking</label>' +
      ...
      '<button class="btn btn-sm" id="sTrackCopy">Copy the address</button></div>';
...
function invoiceForm(client, caseRow) {
  ...
  modal('Raise an invoice',
    (client ? '<p class="muted small">For <b>' +
      esc(client.first_name + ' ' + client.last_name) + '</b>.</p>' +
      '<input type="hidden" id="invClient" value="' + client.id + '">' +
      (caseRow ? '<input type="hidden" id="invCase" value="' + caseRow.id + '">' : '')
      : '<div class="field"><label class="lbl" for="invName">Bill to</label>' +
        '<input class="inp" id="invName" placeholder="Their name"></div>') +
    '<div class="grid-3">' +
    ...
    '<div class="field"><label class="lbl" for="invDate">Date</label>' +
    '<input class="inp" id="invDate" type="date" value="' + today() + '"></div>' +
    ...
    '<textarea class="inp" id="invNotes" placeholder="Leave blank to use the standard wording"></textarea></div>',
    '<button class="btn" onclick="closeModal()">Cancel</button>' +
...
function editInvoice(v) {
  modal('Edit invoice ' + v.number,
    ...
    '<div class="field"><label class="lbl" for="invDate">Date</label>' +
    '<input class="inp" id="invDate" type="date" value="' + esc(v.issue_date) + '"></div>' +
    '<div class="field"><label class="lbl" for="invTax">Tax %</label>' +
  ...
  $('invSave').onclick = function () {
    ...
    api('/api/invoices/' + v.id, {
      bill_name: val('invName'), currency: val('invCurrency'),
