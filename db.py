...
# --------------------------------------------------------------------------
def translate(sql):
    """Turn `?` placeholders into the `%s` psycopg expects.

    A literal per-cent sign has to be doubled at the same time. Once `%s` is
    the placeholder marker, psycopg reads any other `%` as the start of one —
    so a perfectly ordinary `LIKE 'track:%'` is rejected outright. Doubling it
    here means SQL can be written the same way for both engines.
    """
...
class Conn:
    ...
    def __init__(self, raw, is_pg):
        self.raw = raw
        self.is_pg = is_pg
        # Never pooled unless something explicitly says so. The pool belongs
        # to the primary database alone: a standby connection returned into it
        # would be handed to the next request, which would then read from — and
        # could write to — the backup copy.
        self.no_reuse = True
    ...
    def close(self):
        """Give the connection back rather than throwing it away.

        Opening a Postgres connection means a TCP round trip, a TLS handshake
        and authentication. Over a local socket that is free; to a hosted
        database across the internet it is most of the time a small request
        takes. Handing the connection back to a small pool removes that cost
        from every click.
        """
        if self.is_pg and not self.no_reuse and _release(self):
            return
        try:
            self.raw.close()
        except Exception:
    ...
    def discard(self):
        """Close for good — used when a connection is known to be broken."""
        self.no_reuse = True
        try:
            self.raw.close()
        except Exception:
...
# ---- a small pool of connections to the primary --------------------------
POOL_MAX = int(os.environ.get("DB_POOL", "6") or 6)
_pool = []
_pool_lock = threading.Lock()


def _release(conn):
    """Return a connection to the pool. False means caller should close it."""
    if len(_pool) >= POOL_MAX:
        return False
    try:
        # Never hand back a connection mid-transaction: the next request
        # would inherit whatever the last one left behind.
        conn.raw.rollback()
    except Exception:
        return False
    with _pool_lock:
...
def _take():
    with _pool_lock:
        while _pool:
            conn = _pool.pop()
            if not getattr(conn.raw, "closed", False):
                return conn
    return None
...
def connect(dsn=None):
    """A connection to the primary (or to an explicit dsn).

    Pooled connections are only used for the primary. An explicit dsn — the
    standby during a backup copy — is always opened fresh, since it is a
    ...
    if target:
        if dsn is None:
            ...
            if spare is not None:
                try:
                    spare.raw.execute("SELECT 1").fetchone()
                    return spare
                except Exception:
                    # stale after an idle period; fall through and open a new one
                    spare.discard()
        last = None
        for attempt in range(3):
            try:
                conn = _open_pg(target)
                conn.no_reuse = dsn is not None      # only the primary pools
                return conn
...
INDEXES = [
    ...
    "CREATE INDEX IF NOT EXISTS ix_rec_user     ON recovery_codes(user_id)",
    "CREATE INDEX IF NOT EXISTS ix_req_service  ON service_requirements(service)",
    "CREATE INDEX IF NOT EXISTS ix_fee_service  ON service_fees(service)",
    "CREATE INDEX IF NOT EXISTS ix_inv_client   ON invoices(client_id)",
    "CREATE INDEX IF NOT EXISTS ix_inv_date     ON invoices(issue_date)",
    "CREATE INDEX IF NOT EXISTS ix_item_invoice ON invoice_items(invoice_id)",
    "CREATE INDEX IF NOT EXISTS ix_form_service ON forms(service)",
...
# existing table exactly as it is, so a database created by an earlier version
# would never gain these. Each is added on boot if missing, which makes an
# upgrade nothing more than a redeploy.
ADDED_COLUMNS = [
    ("users",    "totp_secret",  "TEXT DEFAULT ''"),
    ("users",    "totp_enabled", "INTEGER NOT NULL DEFAULT 0"),
    ("sessions", "pending",      "INTEGER NOT NULL DEFAULT 0"),
    ("appointments", "reminder_sent", "TEXT DEFAULT ''"),
    ("payments", "invoice_id", "INTEGER"),
    ("appointments", "mode", "TEXT NOT NULL DEFAULT 'office'"),
]
...
def existing_columns(conn, table):
    ...
    try:
        ...
        return set(str(r.get("name") or r.get("column_name") or "") for r in rows)
    except Exception:
        conn.rollback()
        return set()
...
def add_missing_columns(conn):
    ...
    for table, column, spec in ADDED_COLUMNS:
        ...
        try:
            ...
            conn.rollback()
            # a parallel boot may have added it a moment earlier
            if column not in existing_columns(conn, table):
                raise RuntimeError(
                    f"Could not add {table}.{column}: {exc}") from exc
    return added
...
def table_counts(conn):
    ...
    Twenty-one separate counts meant twenty-one trips to a database that may
    be on another continent. One UNION does the same work in one.
    """
    parts = [f"SELECT '{t}' AS t, COUNT(*) AS n FROM {t}" for t in TABLE_NAMES]
    try:
        rows = conn.query(" UNION ALL ".join(parts))
        return {r["t"]: r["n"] for r in rows}
    except Exception:
        # A table missing on an older database shouldn't blank the whole page.
        conn.rollback()
        counts = {}
        for t in TABLE_NAMES:
            try:
                counts[t] = conn.scalar(f"SELECT COUNT(*) AS n FROM {t}")
            except Exception:
                conn.rollback()
                counts[t] = None
        return counts
...
def run_mirror():
    ...
    try:
        ...
        dst.commit()
        # A standby created by an earlier version is missing whatever has been
        # added since. Without this the copy fails on the first new column.
        add_missing_columns(dst)
