...
To put it online, follow [DEPLOY.md](DEPLOY.md). It covers how the first owner
account is created.
...
Commit a file named `logo.png` — either at the top level of the repository or
inside `static/`; both are served. It appears on the sign-in panel and in the
sidebar, and is scaled to fit rather than stretched.

If the file is not there, a drawn crest is shown instead, so nothing looks
broken while you are still finding the artwork. A wide, transparent PNG works
best; the sidebar sits on a dark background, so the logo is given a small white
plate behind it.
