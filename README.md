...
19. [Owner approval](#19-owner-approval)
20. [Removing things](#20-removing-things)
21. [Security](#21-security)
22. [When something goes wrong](#22-when-something-goes-wrong)
23. [The files in this repository](#23-the-files-in-this-repository)
...
Two optional images. The server checks whether they are actually deployed and
marks the page accordingly — there is no guessing, and nothing a cached reply
can get wrong. If the background is missing, open the sign-in page, view
source, and look at the `<html>` tag: `data-photo="0"` means the portal cannot
find `login.png` in the repository, `data-photo="1"` means it can.

Both are optional. Commit either at the top level of the repository or inside
...
**The services themselves** are yours to manage: add one, rename it, retire it
when you stop offering it, or delete it outright. Renaming does not touch cases
already open — their service is the wording agreed with that client at the time.
Deleting one that cases still use warns you first and suggests retiring, which
hides it from the lists while leaving those cases intact.
...
Set it up in **Settings → Consultation reminders**. The panel says plainly
whether email is ready and, if not, exactly which of the three pieces is still
missing.

**Getting the password from Zoho.** Sign in at `accounts.zoho.com` as the
office address, go to **Security → Application Specific Passwords → Generate
New Password**, name it something you will recognise, and copy the password —
Zoho will not show it again. It is an *app password*, not your mailbox
password; Zoho refuses ordinary passwords for SMTP once multi-factor
authentication is on, and recommends app passwords for mail clients either
way. Make sure IMAP/SMTP access is enabled under **Zoho Mail → Settings → Mail
Accounts**, or there is no route to send through.

**Putting it in.** Server `smtp.zoho.com`, port `465`, username and send-from
both the office address. Paste the app password and press **Send a test
message** — that saves everything on the page, stores the password, and then
tries to send, so a value you have just typed is the one actually tested. The
password is stored once and never shown again.

**If the test fails**, the message says why rather than failing quietly. Two
common causes: an account in Zoho's EU data centre needs `smtp.zoho.eu` rather
than `.com`, and sending from an address other than the username is refused by
Zoho as relaying unless "Send mail as" is configured.

Once a test message arrives, switch **Reminders** on. **Send any due now** runs
a sweep by hand so you can watch a real one go out.
...
With this on — and it starts on — **every change an administrator makes waits
for the owner**. Nothing is written until you agree. It appears under
**Approvals**, described in plain words rather than as raw data: *Add a client
— Thabo Mokoena*, *Remove a case*, *Add a payment (500)*. You can see exactly
what was asked for, approve it, or turn it down with a reason the person will
see.

Three things this deliberately does not hold, because otherwise an
administrator could not get into the portal to do the work: signing in and out,
changing their own password, and setting up their own two-step security.
Reading is never held either — only changes.

**A forbidden action is refused, not queued.** Things an administrator may
never do — creating staff accounts, changing settings, deleting a client,
running a backup — come back as *not allowed* rather than sitting in your queue
dressed up as ordinary work. The portal runs the real permission check first
and only then holds the change.

**Approved work is credited to the person who did it.** The activity log shows
the administrator creating the client and the owner approving it, which is what
actually happened.

**Worth knowing before you rely on it.** With approval on, a client sitting in
the office cannot be added until somebody presses the button. If that is not
practical for how the office runs, Settings lets you switch it off and
administrators work directly — everything they do is still in the activity log
either way.
...
Everything in the portal can be removed. Who may do it depends on what it is.

**Anyone with an administrator account** can remove an enquiry, a task, a
consultation, a document, a form, and an entry in the requirements-and-fees
catalogue. They can also **void** a payment, which is usually what you want:
the entry stays visible on the file and drops out of every total, so the books
still explain themselves.

**Only the owner** can remove a client, a case, a staff account, an invoice, a
receipt, a payment outright, or a single entry from a file's history.

**Invoices and receipts** are removed by opening the document from Accounts.
An invoice offers **Cancel it** as well as **Delete**, and cancelling is nearly
always the better answer for anything that has already left the office: the
number stays in the book marked cancelled and drops out of every total, which
is what an accountant expects — a missing number raises questions a cancelled
one does not. Deleting a receipt removes only the paperwork; the payment it
posted stays in the ledger, because that is the record of money actually
received. Remove that separately on the Payments page if the money never
arrived. A number is never reused either way. These
either destroy a record or change the money, so they sit with one person.

Every removal is written to the activity log with who did it and when, so a
deletion is never silent even when the record itself has gone.
...
Every click used to open a fresh connection to the database. Over a local file
that is free; to a database across the internet it is a TCP round trip, a TLS
handshake and an authentication — most of the time a small request takes. The
portal now keeps a handful of connections open and reuses them, so that cost is
paid once rather than on every page. A hundred requests use two connections
instead of a hundred.

A connection is only reused for the primary database, never the standby, and
one that fails mid-request is thrown away rather than handed to the next
person.

**Fewer questions per page.** Counting eight case stages meant eight separate
trips to the database; twelve months of trend meant thirty-six. Those are now
single grouped queries. The dashboard went from 64 questions to 21, analytics
from 58 to 14, the books from 63 to 8, and the System page from 23 to 3 —
the same answers, a fraction of the waiting.

**The browser stops re-downloading the portal.** Stylesheets and scripts carry
a tag that changes only when the file does, so a repeat visit asks "has this
changed?" and is told no in a few bytes instead of fetching 190 KB again — and
a deploy is picked up immediately rather than after a cache expires. Set `DB_POOL` in Render to change how many are kept; six is the
default and is comfortable for a practice this size.

If the portal still feels slow to wake, that is the free plan sleeping rather
than the database — the UptimeRobot ping described earlier is the answer.
...
## 23. The files in this repository
