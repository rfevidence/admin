...
st, d = call("POST", "/api/clients",
             {"first_name": "Expiry", "last_name": "Check",
              "permit_expiry": _tomorrow, "passport_expiry": "2020-01-01"}, T)
CID = d.get("id")
...
forbidden = [
    ...
    ("remove a colleague", "DELETE", "/api/users/1", {}),
]
...
st, d = call("GET", "/export/audit?from=2000-01-01&to=2030-01-01", None, T2)
ok("an advisor cannot export the office reports", st == 403, st)
st, d = call("GET", "/api/clients", None, T2)
ok("  but casework is still theirs to do", st == 200, st)
